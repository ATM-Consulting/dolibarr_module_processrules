...
/**
 *  \file       processstep_note.php
 *  \ingroup    processrules
 *  \brief      Card with notes on processstep
 */
...
$hookmanager->initHooks(array('processstepnote','globalcard'));     // Note that conf->hooks_modules contains array
...
//$result = restrictedArea($user, 'processstep', $id);
...
$help_url='';
llxHeader('', $langs->trans('processStep'), $help_url);
...
if ($id > 0 || ! empty($ref))
{
	...
	dol_fiche_head($head, 'note', $langs->trans("processStep"), -1, $picto);
	...
	if (! empty($conf->projet->enabled))
	{
	    ...
	    if ($user->rights->processrules->creer)
	    {
	        if ($action != 'classify')
